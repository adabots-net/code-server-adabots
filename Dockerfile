...
RUN sudo apt-get update && sudo apt-get install -y --no-install-recommends wget unzip libc6-dev libssl-dev libcrypto++-dev libncurses5 \
  && sudo apt-get clean && sudo rm -rf /var/cache/apt/* && sudo rm -rf /var/lib/apt/lists/* && sudo rm -rf /tmp/*

# clone AdaBots repo
RUN mkdir ~/.ssh
RUN ssh-keyscan -t rsa github.com >> ~/.ssh/known_hosts
RUN git clone -b coder --progress https://github.com/TamaMcGlinn/AdaBots_examples ~/adabots
...
RUN wget https://raw.githubusercontent.com/alire-project/alire/v1.2.2/scripts/alr-completion.bash -O ~/alr-completion.bash
RUN echo 'source ~/alr-completion.bash' >> ~/.bashrc
